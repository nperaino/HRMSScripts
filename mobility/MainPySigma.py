...
print('Enter 4 for a single PSA')
tempType = input('Enter Selection:')
print(tempType)
print('')
print('')
print('')
print('Please Enter buffer gas radius.  Use 1.09 if you are not sure.')
...
if tempType == '1':
    ...
    tempK = float(input('Temperature: '))
    SF = shapeFactor(geometry, acc)
    PA = PSA(geometry, temp=tempK, accuracy=acc, buffr=bufferRadius , maxCycles=maxC)
    correctedArea = float(SF)*float(PA)
    print('')
    print('The projection area is: ',PA)
    print('The Shape Factor is: ', SF)
    print('The corrected PSA is: ', correctedArea)
    input('press enter to quit')